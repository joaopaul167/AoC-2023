...
enum PositionType { PART_NUMBER, NUMBER, SYMBOL, NOTHING };
...
public:
  ...
  void addValue(int xs, int ys, int value) { n_map[{xs, ys}] = value; }

  void addSymbol(int xs, int ys, char symbol) { s_map[{xs, ys}] = symbol; }

  bool haveSymbolAround(int xs, int ys, int n_len) {
    if (s_map[{xs, ys - 1}] || s_map[{xs, ys + n_len}])
      return true;
    for (int i = ys - 1; i <= ys + n_len; i++) {
      acc++;
      if (s_map[{xs - 1, i}] || s_map[{xs + 1, i}])
        return true;
    }
    return false;
  }
  ...
  void process() {
    ...
    for (auto value : n_map) {
      ...
      if (haveSymbolAround(key.first, key.second,
                           std::to_string(number).length())) {
        sum += number;
      }
    }
    ...
    for (auto symbol : s_map) {
      ...
      for (auto adjNum : adjNumbersPos) {
        if (grid[adjNum.first][adjNum.second].second == PositionType::NUMBER) {
          int completeNumber =
              findCompletePartNumber(adjNum.first, adjNum.second);
